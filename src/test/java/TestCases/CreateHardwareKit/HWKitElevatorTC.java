...
public class HWKitElevatorTC extends BaseTest {

    int Flag = 1;
    public HWKitElevatorTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
    ...
    public void verifyAddHWKit() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("Add Elevator Hardware Kit Test Started");
            ExtentTestManager.getTest().assignCategory("Elevator Solution");
            log.info("Add Hardware Kit For Elevator Test Started");
        }

        ElevatorAdminLoginTC elevatorAdminLoginTC = new ElevatorAdminLoginTC(driver,1);
        elevatorAdminLoginTC.verifyLogin();
