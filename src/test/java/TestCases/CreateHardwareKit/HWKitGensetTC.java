...
import TestCases.Login.AdminLogin.GensetAdminLoginTC;
import TestCases.Login.AdminLogin.LightingAdminLoginTC;
import com.relevantcodes.extentreports.LogStatus;
import org.openqa.selenium.WebDriver;
import org.testng.Assert;
import org.testng.annotations.Test;

public class HWKitGensetTC extends BaseTest {

    int Flag = 1;
    public HWKitGensetTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
    ...
    public void verifyAddHWKit() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("Add GenSet Hardware Kit Test Started");
            ExtentTestManager.getTest().assignCategory("Smart GenSet Solution");
            log.info("Add Hardware Kit For Smart GenSet Test Started");
        }

        GensetAdminLoginTC gensetAdminLoginTC = new GensetAdminLoginTC(driver, 1);
        gensetAdminLoginTC.verifyLogin();
        ...
        //Step 6: Enter Name Lighting Hardware Kit Name
        Assert.assertTrue(commonMethods.enterTextInInputField(hardwareKitPO.name_Txt_Ele, hardwareKitPO.name), "Unable to Enter Lighting Hardware Kit Name");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Enter Lighting Hardware Kit Name", "Test Data: "+hardwareKitPO.name);

        //Step 7: Enter Note
        Assert.assertTrue(commonMethods.enterTextInInputField(hardwareKitPO.note_Txt,"Note Text"), "Unable to Enter Note");
