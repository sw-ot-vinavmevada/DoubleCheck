...
public class HWKitLightingTC extends BaseTest {

    int Flag = 1;
    public HWKitLightingTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
    ...
    public void verifyAddHWKit() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("Add Lighting Hardware Kit Test Started");
            ExtentTestManager.getTest().assignCategory("Lighting Solution");
            log.info("Add Hardware Kit For Green House Test Started");
        }

        LightingAdminLoginTC lightingAdminLoginTC = new LightingAdminLoginTC(driver, 1);
        lightingAdminLoginTC.verifyLogin();
