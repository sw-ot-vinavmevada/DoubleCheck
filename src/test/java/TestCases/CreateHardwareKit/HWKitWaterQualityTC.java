...
import TestCases.Login.AdminLogin.WaterQualityAdminLoginTC;
import com.relevantcodes.extentreports.LogStatus;
import org.openqa.selenium.WebDriver;
import org.testng.Assert;
import org.testng.annotations.Test;

public class HWKitWaterQualityTC extends BaseTest {

    int Flag = 1;
    public HWKitWaterQualityTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
    ...
    public void verifyAddHWKit() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("Add Water Quality Hardware Kit Test Started");
            ExtentTestManager.getTest().assignCategory("Water Quality Solution");
            log.info("Add Hardware Kit For Green House Test Started");
        }

        WaterQualityAdminLoginTC waterQualityAdminLoginTC = new WaterQualityAdminLoginTC(driver, 1);
        waterQualityAdminLoginTC.verifyLogin();
        ...
        Thread.sleep(5000);
        Assert.assertTrue(commonMethods.clickOnLinkOrButton(hardwareKitPO.hw_Icon_WQ), "Unable to click on Hardware Kit Icon");
        ...
        ExtentTestManager.getTest().log(LogStatus.PASS, "Click on Add Kit button", "User got redirected on add Hardware Kit page");
        ...
        //Step 6: Enter Name Water Hardware Kit Name
        Assert.assertTrue(commonMethods.enterTextInInputField(hardwareKitPO.name_Txt_Ele, hardwareKitPO.name), "Unable to Enter Water Quality Hardware Kit Name");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Enter Water Quality Hardware Kit Name", "Test Data: "+hardwareKitPO.name);
