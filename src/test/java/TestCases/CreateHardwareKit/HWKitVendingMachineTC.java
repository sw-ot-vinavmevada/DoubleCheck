...
import TestCases.Login.AdminLogin.AirQualityAdminLoginTC;
import TestCases.Login.AdminLogin.VendingAdminLoginTC;
import com.relevantcodes.extentreports.LogStatus;
import org.openqa.selenium.WebDriver;
import org.testng.Assert;
import org.testng.annotations.Test;

public class HWKitVendingMachineTC extends BaseTest {

    int Flag = 1;
    public HWKitVendingMachineTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
    ...
    public void verifyAddHWKit() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("Add Vending Machine Hardware Kit Test Started");
            ExtentTestManager.getTest().assignCategory("Vending Machine Solution");
            log.info("Add Hardware Kit For Vending Machine Test Started");
        }

        VendingAdminLoginTC vendingAdminLoginTC = new VendingAdminLoginTC(driver,1);
        vendingAdminLoginTC.verifyLogin();
        ...
        Thread.sleep(5000);
        Assert.assertTrue(commonMethods.clickOnLinkOrButton(hardwareKitPO.hw_Icon_Air), "Unable to click on Hardware Kit Icon");
        ...
        Assert.assertTrue(commonMethods.enterTextInInputField(hardwareKitPO.name_Txt_Ele, hardwareKitPO.name), "Unable to  Enter Name Elevator Hardware Kit Name");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Enter Elevator Hardware Kit Name", "Test Data: "+hardwareKitPO.name);
