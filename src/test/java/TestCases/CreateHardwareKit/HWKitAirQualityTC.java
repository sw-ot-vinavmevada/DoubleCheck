...
import TestCases.Login.AdminLogin.ElevatorAdminLoginTC;
import com.relevantcodes.extentreports.LogStatus;
import org.openqa.selenium.WebDriver;
import org.testng.Assert;
import org.testng.annotations.Test;

public class HWKitAirQualityTC extends BaseTest {

    int Flag = 1;
    public HWKitAirQualityTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
    ...
    public void verifyAddHWKit() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("Add Air Quality Hardware Kit Test Started");
            ExtentTestManager.getTest().assignCategory("Air Quality Solution");
            log.info("Add Hardware Kit For Air Quality Test Started");
        }

        AirQualityAdminLoginTC airQualityAdminLoginTC = new AirQualityAdminLoginTC(driver,1);
        airQualityAdminLoginTC.verifyLogin();
