...
import TestCases.CreateHardwareKit.HWKitGreenHouseTC;
import com.relevantcodes.extentreports.LogStatus;
import org.openqa.selenium.WebDriver;
import org.testng.Assert;
import org.testng.annotations.Test;

public class CreateGreenHouseUserTC extends BaseTest {

    int Flag = 1;

    public CreateGreenHouseUserTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
    ...
    @Test
    public void VerifyToCreateGreenHouseUser() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("To Verify user is able to add green house solutions");
            ExtentTestManager.getTest().assignCategory("Smart Green House Solution");
            log.info("Create Company User For Smart Green House Solution Test Started");
        }


        //Pre Condition:- Create Hardware Kit
        HWKitGreenHouseTC hwKitGreenHouseTC = new HWKitGreenHouseTC(driver,1);
        hwKitGreenHouseTC.verifyAddHWKit();
        ...
        Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.email_Txt, companyUserLoginPO.email_Str_GH), "Unable to Enter Email Address");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Enter Email Address", "Test Data: "+companyUserLoginPO.email_Str_GH);
