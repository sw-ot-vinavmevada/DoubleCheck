...
import TestCases.CreateHardwareKit.HWKitRefrigeratorTC;
import com.relevantcodes.extentreports.LogStatus;
import org.openqa.selenium.WebDriver;
import org.testng.Assert;
import org.testng.annotations.Test;

public class CreateRefrigeratorUserTC extends BaseTest {

    int Flag = 1;

    public CreateRefrigeratorUserTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
...
*/
    ...
    public void VerifyCreateCompanyUser_111() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("Add Company User For Refrigerator Test Started");
            ExtentTestManager.getTest().assignCategory("Refrigerator Solution");
            log.info("Create Company User For Refrigerator Solution Test Started");
        }


        //Pre Condition:- Create Hardware Kit
        HWKitRefrigeratorTC hwKitRefrigeratorTC = new HWKitRefrigeratorTC(driver,1);
       // hwKitRefrigeratorTC.verifyAddHWKit();
        ...
        Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.email_Txt, companyUserLoginPO.email_Str_Ref), "Unable to Enter Email Address");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Enter Email Address", "Test Data: "+companyUserLoginPO.email_Str_Ref);
