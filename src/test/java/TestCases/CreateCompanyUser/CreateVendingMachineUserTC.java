...
import TestCases.CreateHardwareKit.HWKitVendingMachineTC;
import com.relevantcodes.extentreports.LogStatus;
import org.openqa.selenium.WebDriver;
import org.testng.Assert;
import org.testng.annotations.Test;

public class CreateVendingMachineUserTC extends BaseTest {

    int Flag = 1;

    public CreateVendingMachineUserTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
...
*/
    ...
    @Test
    public void verifyCreateCompanyUser_VendingMachine() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("To Verify user is able to add vending machine solutions");
            ExtentTestManager.getTest().assignCategory("Vending Machine Solution");
            log.info("Create Company User For Vending Machine Solution Test Started");
        }


        //Pre Condition:- Create Hardware Kit
        HWKitVendingMachineTC hwKitVendingMachineTC = new HWKitVendingMachineTC(driver,1);
        //hwKitVendingMachineTC.verifyAddHWKit();
        ...
        Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.email_Txt, "testemailsudmos@mailinator.com"), "Unable to Enter Email Address");
