...
public class CreateElevatorUserTC extends BaseTest {

    int Flag = 1;

    public CreateElevatorUserTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
...
*/
    ...
    @Test
    public void VerifyCreateCompanyUser_111() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("To Verify User is able to add elevator solution");
            ExtentTestManager.getTest().assignCategory("Elevator Solution");
            log.info("Create Company User For Elevator Solution Test Started");
        }


        //Pre Condition:- Create Hardware Kit
        HWKitElevatorTC hwKitElevatorTC = new HWKitElevatorTC(driver,1);
        //hwKitElevatorTC.verifyAddHWKit();
        ...
        Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.email_Txt, companyUserLoginPO.email_Str_Ele), "Unable to Enter Email Address");
...
/*
        Thread.sleep(6000);
                //Step 1:Click on Link Create New Account
                Assert.assertTrue(commonMethods.clickByJS(companyUserPO.newAccount_Lnk), "Click on Link Create New Account");
                Thread.sleep(3000);
                //Step 2:Enter Company Name
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.companyName_Txt, companyUserPO.compName), "Enter Company Name");
                //Step 3:Enter Contact Number
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.contactNumber_Txt, "9876543210"), "Enter Contact Number");
                //Step 4:Enter Address
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.address_Txt, "Test Address"), "Enter Address");
                //Step 5:Select Country
                Assert.assertTrue(commonMethods.selectValueFromDD(companyUserPO.country_DD, "Australia", companyUserPO.country_Value), "Select Country");
                Thread.sleep(6000);
                //Step 6:Select state
                Assert.assertTrue(commonMethods.selectValueFromDD(companyUserPO.state_DD, "Melbourne", companyUserPO.state_Value));
                //Step 7:Enter City
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.city_Txt, "Sydney"), "Enter City");
                //Step 8:Select TimeZone
                Assert.assertTrue(commonMethods.clickOnLinkOrButton(companyUserPO.timeZone_DD));
                Assert.assertTrue(commonMethods.clickOnLinkOrButton(companyUserPO.timeZone_Value), "Select TimeZone");
                //Step 9:Enter ZipCode
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.zip_Txt, "345634"), "Enter ZipCode");
                //Step 10:Enter First Name
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.fName_Txt, "Tom"+companyUserPO.fName_Str), "Enter First Name");
                //Step 11:Enter Last Name
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.lName_Txt, "Miller"+companyUserPO.lName_Str), "Enter Last Name");
                //Step 12:Enter Email Address
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.email_Txt, companyUserPO.email_Str), "Enter Email Address");
                //Step 13:Enter Password
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.pass_Txt, "Softweb#123"), "Enter Password");
                //Step 14:Click on Next Button
                Assert.assertTrue(commonMethods.clickOnLinkOrButton(companyUserPO.Next_Btn), "Click on Next Button");
                Thread.sleep(3000);
                //Step 15:Select Basic Plan
                Assert.assertTrue(commonMethods.clickOnLinkOrButton(companyUserPO.basic_Plan), "Select Basic Plan");
                //Step 16:Enter Card Name
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.cardName_Txt, companyUserPO.cardHolderNameStr), "Enter Card Name");
                companyUserPO.switchFrame();
                //Step 17:Enter Card Number
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.cardNumber_Txt, companyUserPO.cardNumberStr), "Enter Card Number");
                //Step 17:Enter Card Exp Date
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.cardExpDate_Txt, companyUserPO.expDateStr), "Enter Card Exp Date");
                //Step 17:Enter Card CVV
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.cardCVV_Txt, companyUserPO.cvcCodeStr), "Enter Card CVV");
                //Step 17:Enter Card Post
                Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.cardPostal_Txt, companyUserPO.zipStr), "Enter Card Post");
                //Step 14:Click on Confirm Button
                commonMethods.switchToDefaultContent();
                Assert.assertTrue(commonMethods.clickOnLinkOrButton(companyUserPO.confirm_Btn), "Click on Confirm Button");
