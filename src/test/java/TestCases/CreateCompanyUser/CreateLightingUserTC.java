...
public class CreateLightingUserTC extends BaseTest {

    int Flag = 1;

    public CreateLightingUserTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
...
*/
    ...
    @Test
    public void VerifyCreateCompanyUser_Lighting() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("To verify user is able to add lighting solutions");
            ExtentTestManager.getTest().assignCategory("Elevator Solution");
            log.info("Create Company User For Lighting Solution Test Started");
        }


        //Pre Condition:- Create Hardware Kit
        HWKitLightingTC hwKitLightingTC = new HWKitLightingTC(driver, 1);
        //hwKitLightingTC.verifyAddHWKit();
        ...
        Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.email_Txt, companyUserLoginPO.email_Str_Lig), "Unable to Enter Email Address");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Enter Email Address", "Test Data: "+companyUserLoginPO.email_Str_Lig);
