...
import TestCases.CreateHardwareKit.HWKitGensetTC;
import TestCases.CreateHardwareKit.HWKitLightingTC;
import com.relevantcodes.extentreports.LogStatus;
import org.openqa.selenium.WebDriver;
import org.testng.Assert;
import org.testng.annotations.Test;

public class CreateGensetUserTC extends BaseTest {

    int Flag = 1;

    public CreateGensetUserTC(WebDriver driver, int flag) {
        this.driver = driver;
        this.Flag = flag;
    }
...
*/
    ...
    @Test
    public void verifyCreateCompanyUser_GenSet() throws Exception {
        if (Flag > 0) {
            ExtentTestManager.getTest().getTest().setName("To Verfify User is able to add genset solutions");
            ExtentTestManager.getTest().assignCategory("Smart GenSet Solution");
            log.info("Create Company User For GenSet Solution Test Started");
        }


        //Pre Condition:- Create Hardware Kit
        HWKitGensetTC hwKitGensetTC = new HWKitGensetTC(driver, 1);
        //hwKitGensetTC.verifyAddHWKit();
        ...
        Assert.assertTrue(commonMethods.enterTextInInputField(companyUserPO.email_Txt, companyUserLoginPO.email_Str_GenSet), "Unable to Enter Email Address");
        ExtentTestManager.getTest().log(LogStatus.PASS, "Enter Email Address", "Test Data: "+companyUserLoginPO.email_Str_GenSet);
